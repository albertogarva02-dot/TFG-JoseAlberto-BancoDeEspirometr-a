...
class GUI:
    def __init__(self):
        ...
        <div style="margin: 10px;">
            ...
            Departamento de Ingeniería mecánica, máquinas y motores térmicos y fluidos - Escuela de Ingeniería Industrial - Universidad de Vigo</p>
            <h3>Directores</h3>
            <p><b>Jesús Vence Fernández y Eduardo Suarez Porto</b><br>
...
#------------Botones emergencia-------------------------
    ...
    def __precargar_imagenes_ansys(self):
        ...
        for i in range(max_in + 1):
            self.__nombre = ruta_recurso( f"{ruta_in}/velocidad_in_{i:04d}_recortada_reflejo.png")
            if QFile.exists(self.__nombre):
                pix = self.__procesar_imagen_ansys(self.__nombre)
                self.__cache_ansys_in[i] = pix

        for i in range(max_out + 1):
            self.__nombre = ruta_recurso(f"{ruta_out}/velocidad_{i:04d}_recortada_reflejo.png")
            if QFile.exists(self.__nombre):
                pix = self.__procesar_imagen_ansys(self.__nombre)
...
#-------------------------------------VENTANA_CONTROL_MOTOR--------------------------------------------------------------
    ...
    def movimientopistones(self, posicion):
        ...
        y_min_inicio=221 
        y_max_inicio=311 
        y_min_fin_inicio=324 
        y_max_fin_inicio=414 
        y_min_esquinero_inicio=322 
        y_max_esquinero_inicio=412 
        y_min_cremallera=324 
        y_max_cremallera=414  
